...
QueryBuilder& QueryBuilder::where(const std::string& condition, const std::vector<SqlValue>& params) {
...
const std::vector<SqlValue>& QueryBuilder::getParams() const {
    static std::vector<SqlValue> combined;
    combined.clear();
    switch (_type) {
        case Type::SELECT:
        case Type::DELETE:
